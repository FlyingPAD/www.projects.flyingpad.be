...
export class DiceRollComponent {
  ...
  maleStartRollMobile(): void {
    this.maleStartRoll();
  }

  diceRoll(max: number): number {
    this.currentDiceRoll = Math.floor(Math.random() * max);
    return this.currentDiceRoll;
  }

  diceRollCheck(): void {
    if (this.resultFemale === undefined || this.resultMale === undefined) return;
